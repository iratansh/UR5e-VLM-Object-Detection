...
# Import the original UnifiedVisionSystem
...
class UnifiedVisionSystemSim(UnifiedVisionSystem):
    ...
    def _sim_color_callback(self, msg: Image):
        ...
        try:
            ...
            self.logger.error(f"Error converting color image: {e}")
    ...
    def _run_simulation_pipeline(self):
        ...
        if not vlm_detections_2d:
            self.logger.warning("VLM detection yielded no results")
        ...
        if not valid_detection_3d:
            self.logger.warning("No valid 3D object found")
        ...
        if not self._validate_robot_pose(target_pose_matrix):
            self.logger.error("Generated pose is invalid")
            return
